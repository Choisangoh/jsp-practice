...
public class BoardUpdateFormServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 1. boarddetail.jsp에 목적지를 현재 서블릿으로 하고 post방식 전달을 하는 
		// form과 submit버튼 수정하기 만들기 
	    // 2. doPost에서 실제로 수정하기를 눌렀을때 반으이 있는지 체크
		...
		// 기입된 상태가 되려면 먼저 입력된 글 번호에 해당하는 글정보를 DB에서 자바로 가져와야한다.
		// 글번호 입력 시 해당번호 글의 정보를 가져오는 로직은 BoardDAO의 getBoardDetail()에 작성되어 있다.
